...
export default function ProductCard({ produto }) {
  ...
  return (
    <div className="group bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 relative">
      <Link
      ...
      >
        <div className="aspect-[4/3] overflow-hidden bg-gray-100 relative">
          ...
          <button
            ...
            className="absolute top-2 right-2 w-8 h-8 flex items-center justify-center rounded-full bg-white/80 hover:bg-white shadow-sm transition-all z-10"
            aria-label={fav ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}
          >
            <svg
              className={`w-5 h-5 transition-colors ${fav ? 'text-red-500' : 'text-gray-400 hover:text-red-400'}`}
      ...
      <div className="p-4">
        <h3 className="font-semibold text-gray-900 text-sm line-clamp-2 leading-snug min-h-[2.5em]">
          {produto.nome}
        </h3>
        {produto.descricao && (
